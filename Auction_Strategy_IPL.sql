...
-----------------------------------------------
/*
Our first priority is to get 2-3 players with high S.R who have faced at least 500 balls.And to 
...
/* 
Now we need to get 2-3 players with good Average who have played more the 2 ipl seasons.
And to do that we have to make a list of 10 players you want to bid in the auction so that when you try to grab them 
...
/*
Now we need to get 2-3 Hard-hitting players who have scored most runs in boundaries and have played more 
the 2 ipl season. To do that we have to make a list of 10 players you want to bid in the auction so that when you 
try to grab them in auction we should not pay the amount greater than we have in the purse for a particular player.
*/
select a.batsman,
...
/*
Our first priority is to get 2-3 bowlers with good economy who have bowled at least 500 balls in IPL so far.
To do that we have to make a list of 10 players you want to bid in the auction so that when you try to grab them in 
auction we should not pay the amount greater than you have in the purse for a particular player.
*/
...
/*
Now we need to get 2-3 bowlers with the best strike rate and who have bowled at least 500 balls in IPL so far.
To do that we have to make a list of 10 players we want to bid in the auction so that when you try to grab them in 
...
/*
Now we need to get 2-3 All_rounders with the best batting as well as bowling strike rate and who have faced at least 
500 balls in IPL so far and have bowled minimum 300 balls.To do that we have to make a list of 10 players you want 
to bid in the auction so that when you try to grab them in auction we should not pay the amount greater than we have 
in the purse for a particular player.
*/
...
-- 1. Let's the count of cities that have hosted an IPL match
select count(distinct city) as city_count from ipl_matches

/*
2.Creating table deliveries_v02 with all the columns of the table ‘deliveries’ and an additional column ball_result containing 
values boundary, dot or other depending on the total_run (boundary for >= 4, dot for 0 and other for any other number)
(To achive this we will use CASE WHEN statement is used to get condition based results)
(and To convert the output data of the select statement into a table, we will be using a subquery. Create table table_name 
...
-- Let's see the total number of boundaries and dot balls from the deliveries_v02 table.
select ball_result, count (*) from deliveries_v02 group by ball_result; 

/* Now let's find the total number of boundaries scored by each team from the deliveries_v02 table and order 
...
/* Now try to see the total number of dot balls bowled by each team and order it in descending order of the 
...
--Let's see the total number of dismissals by dismissal kinds where dismissal kind is not NA
select dismissal_kind, count (*) from deliveries where dismissal_kind <> 'NA' group by dismissal_kind order by count desc; 

-- Try to get the top 5 bowlers who conceded maximum extra runs from the deliveries table
select bowler, sum(extra_runs) as total_extra_runs from deliveries group by bowler order by total_extra_runs desc limit 5; 

/* Let's create a table named deliveries_v03 with all the columns of deliveries_v02 table and two additional 
column (named venue and match_date) of venue and date from table matches for further analysis
*/
select * from ipl_matches
...
-- Now let's the total runs scored for each venue and order it in the descending order of total runs scored.
select venue, sum(total_runs) as runs from deliveries_v03 group by venue order by runs desc; 

--Let's try to find out the year-wise total runs scored at Eden Gardens and order it in the descending order of total runs scored.
